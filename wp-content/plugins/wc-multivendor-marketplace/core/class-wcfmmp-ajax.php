...
class WCFMmp_Ajax {
	...
	function wcfmmp_vendor_order_status_update( $order_id, $order_status ) {
		...
		if( $vendor_id ) {
			...
			$sql = 'SELECT product_id, commission_status  FROM ' . $wpdb->prefix . 'wcfm_marketplace_orders AS commission';
			$sql .= ' WHERE 1=1';
			...
			if( $is_all_complete && in_array( $status, apply_filters( 'wcfm_change_main_order_on_child_order_statuses', array( 'completed' ) ) ) && apply_filters( 'wcfm_is_allow_mark_complete_main_order_on_all_child_order_complete', false ) ) {
